...
APP="/Applications/Eleven Views Tools (Developer).app"
PROCESS="ElevenViewsToolsDeveloper"
OUT="${1:-$(mktemp -d /tmp/eleven-views-tools-ui-smoke.XXXXXX)}"
mkdir -p "$OUT"
FAILURES=0
...
sleep 1.5
QP=$(ax 'get position of window "Eleven Views Tools"')
if [[ -n "${QP:-}" ]]; then
...
sleep 1.5
SW=$(ax 'get position of window "Eleven Views Tools Settings"')
if [[ -n "${SW:-}" ]]; then
    pass "settings window at $SW"
    screencapture -x "$OUT/settings.png"
    ax 'click button 1 of window "Eleven Views Tools Settings"' >/dev/null
else
