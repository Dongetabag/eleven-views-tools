...
/// Completes an on-disk rename for early Eleven Views development builds.
///
/// The in-app updater shipped in those builds installs the new version AT THE
/// OLD PATH ("/Applications/Eleven Views Tools Utils.app"), so right after updating the
/// app is running from a bundle still named "Eleven Views Tools Utils.app". We rename
/// that bundle to "Eleven Views Tools.app" through a detached helper that runs only after
...
///
/// When we are already running as "Eleven Views Tools.app", we instead retire a stray
...
enum BundleMigration {
    private static let oldName = "Eleven Views Utilities.app"
    private static let newName = "Eleven Views Tools.app"
